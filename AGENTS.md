...
## notes
each time after making a plan or making changes, save a note in `design/develop-notes/` using a date-and-time filename, and write down the plan and the changes you made. also append the key points of the plan and changes to the single rolling file `design/key-changes/key-changes.md` instead of creating a new key-changes file each time. if making changes, read `design/key-changes/key-changes.md` when relevant. file `design/design.md` is the main design document and should be updated when making changes; it records the important points of the tool.
